...
TopologyEditor.prototype.initMenus = function () {
    ...
    // 但凡是 节点菜单下的 编辑，都会打开一个编辑的菜单
    self.nodeMenu.on('click','div[data-edit]',function() {
        let nodes = self.utils.getSelectedNodes()
        ...
        if (node.__type__) {
            ...
            // 表单数据复原
            self.editSyncForm(node)

            typeMap.get(node.__type__) && typeMap.get(node.__type__).toggleModal()
        }
        else alert('不支持编辑')
    })
...
TopologyEditor.prototype.init = function (topologyGuid, backImg, topologyJson) {
    ...
    this.scene.mouseup(function (event) {
        ...
        if (event.button === 2) {                      // 右键菜单
            ...
            console.log(self.xInCanvas,self.yInCanvas)
            if (event.target) {
 ...
 * @param formOptions {Object} key:value，一般是指表单数据
