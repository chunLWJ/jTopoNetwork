...
let extendsFunc = (function(){
     ...
     * }
     */
    function getOffset(start,target,offset = 10,width = 20,height = 20){
        let offsetScale = 100 / offset

        let x = {
            x: ((offsetScale - 1) * start.x + target.x)/ offsetScale - width / 2,
            y: ((offsetScale - 1) * start.y + target.y)/ offsetScale - height / 2,
        }
        let y = {
            x: (start.x + (offsetScale - 1) * target.x)/ offsetScale - width / 2,
            y: (start.y + (offsetScale - 1) * target.y)/ offsetScale - height / 2,
        }
        return {
            x,
            y
     ...
     * @return {number}
     */
    function getAngle(start,target) {
            ...
            this.paintText = function (a, b) {
                ...
                if (4 == b.length && (c = b[1], d = b[2]), this.text && this.text.length > 0) {
                    ...
                    if (a.fillStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")", this.nodeA === this.nodeZ) {
                        ...
                        // a.fillText(this.text, e - g / 2, f - h / 2); // 源代码


                        // 假设两个矩形，后面的话这个宽度应该采取 字体 宽度 和 默认字体高度
                        // 这里可以考虑最佳偏移量，这里定死 百分之 十
                        let offset = 10
                        // 计算两点的偏移百分比
                        let {x , y} =  extendsFunc.getOffset(c,d,offset,20,20)
                        a.fillRect(x.x ,x.y,20,20)
                        a.fillStyle = 'red'
                        a.fillRect(y.x,y.y,20,20)
                    };
