...
TopologyEditor.prototype.initMenus = function () {
    ...
    self.mainMenu.on('click','div[data-type]',function(event){
        let type = $(this).attr('data-type')
        $('.form_title').html('添加')
        $('.node_submit').data('edit', false)
        typeMap.get(type) && typeMap.get(type).toggleModal()
    })

    $('.node_submit').on('click',function(event){
        let type = $(this).attr('data-type')
        if (typeMap.has(type)) {
            let edit = $(this).data('edit')
            // nodeAddEdit(edit,type,self)
            self.nodeAddEdit(type,edit)
        }
    })
        ...
        if (event.button === 2) {                      // 右键菜单
            ...
            if (event.target) {
                if (event.target instanceof JTopo.Node) {
                    let selectedNodes = self.utils.getSelectedNodes()
                    if (selectedNodes.length > 1) position(self.groupMangeMenu,menuX,menuY)
                    else  position(self.nodeMenu,menuX,menuY)
                }
                else if (event.target instanceof JTopo.Link) position(self.lineMenu,menuX,menuY)
                else if (event.target instanceof JTopo.Container) position(self.containerMangeMenu,menuX,menuY)
            } else position(self.mainMenu,menuX,menuY)
            function position(menu,left,top){
                menu.css({
                    left,top
                }).show()
            }
            ...
            console.log(event.target,event.target && event.target.__type)
            if (event.target && event.target.__type === 'circle'){
                event.target.zIndex = -999
                console.log(event.target)
                return
            }
            if (event.target != null &&
                event.target instanceof JTopo.Node &&
                !self.isSelectedMode && editor.stageMode === 'edit') {
    ...
    drawArea.ondrop = function (event) {
        ...
        if (data) {
            ...
            if (datas && datas.length === 3) {
                ...
                // 节点坐标
                let zoom = self.stage.wheelZoom
                console.log(self.stage)
                console.log(self.scene)
                console.log(editor)
                let x = (event.layerX ? event.layerX : event.offsetX) - self.scene.translateX - self.config.nodeDefaultWidth / 2
                let y = (event.layerY ? event.layerY : event.offsetY) - self.scene.translateY - self.config.nodeDefaultHeight / 2
                node.setBound(x,y,self.config.nodeDefaultWidth,self.config.nodeDefaultHeight)
 ...
 * @returns {*}
 */
TopologyEditor.prototype.getLink = function(endNode) {
    let lineLink = {
        line: 'Link',
        foldLine: 'FoldLink',
    ...
    if (this.lineType === 'foldLine' && endNode) link.bundleOffset = this.config.linkOffsetGap    // 折线拐角处的长度
    if (this.lineType === 'flexLine' && endNode) link.offsetGap = this.config.linkOffsetGap

    if (!endNode) {
        link.dashedPattern = 2
        link.shadow = this.config.linkShadow
        ...
        if (this.lineType !== 'curveLine') link.arrowsRadius = this.config.arrowsRadius
    }
...
TopologyEditor.prototype.nodeFormSync = function(type,node,positive){
    let typeOption = typeMap.get(type)
    let form = $(`#${typeOption.prefix + 'form'}`)
    if (!form) return console.warn(`不存在这个表单名称：${typeOption.prefix + 'form'}`)
...
TopologyEditor.prototype.newNode = function(type) {
    ...
    typeOption.formNameEditArr.forEach(item => {
        let formItem = formOption.find(option => option.name === `${typeOption.prefix}${item}`)
        option[item] = (formItem && formItem.value) || this.config[`${type}${item[0].toLocaleUpperCase()}${item.slice(1)}`] || ''
    })
    form[0].reset()
    console.log(`formOption`,option)
    this.addNode(type,option)
}
TopologyEditor.prototype.nodeAddEdit = function(type, edit = false) {
    if (!edit) typeMap.get(type) && this.newNode(type)
        ...
        if (nodes.length === 1)
            this.nodeFormSync(type,this.utils.getSelectedNodes()[0],false)
        console.log('暂时不支持多编辑')
    }
    typeMap.get(type) && typeMap.get(type).toggleModal()
}
